...
import subprocess
import dotenv
from utils.utils import CaptureLogs, stdout_stream, stderr_stream, convert_to_words, extract_text_from_pdf
...
def main():
    ...
    with col1:
        ...
        if assistant == CUSTOM_ASSISTANT_TAG:
            ...
            if process.returncode == 0:
                ...
                st.error(
                    f"Failed to start custom build interface. Error: {stderr.decode('utf-8')}")
    ...
    input_prompt = st.text_area(
        "Enter your prompt below:", value=st.session_state.input_prompt if st.session_state.transcription_complete else "")
    ...
    if st.button("Run") or st.session_state.transcription_complete:
        ...
        if len(input_prompt) > 0:
            ...
            with col1:
                ...
                st.markdown(
                    "<span style='color: white; font-size: 20px;'>{}</span>".format(result), unsafe_allow_html=True)
            ...
            with col2:
                ...
                new_output = current_output[len(
                    st.session_state.previous_output):]
                # Display the new content in Streamlit
                # st.write(new_output)
                st.markdown("<span style='color: white; font-size: 20px;'>{}</span>".format(
                    new_output), unsafe_allow_html=True)
